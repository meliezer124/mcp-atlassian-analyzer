...
class CycleTimeMetric(ApiModel):
    ...
    value_minutes: int | None = Field(
        default=None,
        description="Cycle time in minutes (None if not resolved)"
    )
    formatted: str | None = Field(
        default=None,
        description="Human-readable duration (e.g., '5d 2h 30m')"
    )
    calculated: bool = Field(
        default=False,
        description="Whether the metric could be calculated"
    )
...
class TimeInStatusEntry(ApiModel):
    ...
    percentage: float = Field(
        default=0.0,
        description="Percentage of total time spent in this status"
    )
    visit_count: int = Field(
        default=1,
        description="Number of times the issue was in this status"
    )
...
class TimeInStatusMetric(ApiModel):
    ...
    statuses: list[TimeInStatusEntry] = Field(
        ...
        description="Time breakdown by status"
    )
    total_minutes: int = Field(
        default=0,
        description="Total time across all statuses"
    )
    ...
    def from_api_response(
        ...
        statuses = [
            TimeInStatusEntry.from_api_response(s)
...
class DueDateComplianceMetric(ApiModel):
    ...
    margin_minutes: int | None = Field(
        default=None,
        description="Minutes early (positive) or late (negative)"
    )
    formatted_margin: str | None = Field(
        default=None,
        description="Human-readable margin"
    )
...
class ResolutionTimeMetric(ApiModel):
    ...
    reason: str | None = Field(
        default=None,
        description="Reason if not calculated"
    )
...
class FirstResponseTimeMetric(ApiModel):
    ...
    response_type: str | None = Field(
        default=None,
        description="Type of first response (comment/transition)"
    )
...
class IssueSLABatchResponse(ApiModel):
    ...
    issues: list[IssueSLAResponse] = Field(
        default_factory=list,
        description="List of issue SLA responses"
    )
    ...
    error_count: int = Field(
        default=0,
        description="Number of issues that failed"
    )
    ...
    working_hours_applied: bool = Field(
        default=False,
        description="Whether working hours filter was applied"
    )
    working_hours_config: WorkingHoursConfig | None = Field(
        default=None,
        description="Working hours configuration if applied"
    )
