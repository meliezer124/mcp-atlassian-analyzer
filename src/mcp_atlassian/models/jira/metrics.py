...
class StatusChangeEntry(ApiModel):
    ...
    duration_minutes: int | None = Field(
        default=None,
        description="Total minutes spent in this status"
    )
    duration_formatted: str | None = Field(
        default=None,
        description="Human-readable duration (e.g., '2d 3h 15m')"
    )
    transitioned_by: str | None = Field(
        default=None,
        description="Display name of the user who made the transition"
    )
...
class IssueDatesResponse(ApiModel):
    ...
    status_summary: list[StatusTimeSummary] = Field(
        default_factory=list,
        description="Aggregated time spent in each status"
    )
...
class IssueDatesBatchResponse(ApiModel):
    ...
    issues: list[IssueDatesResponse] = Field(
        default_factory=list,
        description="List of issue date responses"
    )
    ...
    error_count: int = Field(
        default=0,
        description="Number of issues that failed to process"
    )
